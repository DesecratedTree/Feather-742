...
public final class CombatDefinitions implements Serializable {
	...
	public void refreshSpellBook() {
		if (spellBook == 0) {
			player.getPackets().sendConfig(
					1376,
					sortSpellBook | (showCombatSpells ? 0 : 1 << 9)
			...
			player.getPackets().sendConfig(
					1376,
					sortSpellBook << 3 | (showCombatSpells ? 0 : 1 << 16)
							| (showTeleportSpells ? 0 : 1 << 17));
		} else if (spellBook == 2) {
			player.getPackets().sendConfig(
					1376,
					sortSpellBook << 6 | (showCombatSpells ? 0 : 1 << 13)
	...
	public void setAttackStyle(int style) {
		...
		String name = weaponId == -1 ? "" : ItemDefinitions
