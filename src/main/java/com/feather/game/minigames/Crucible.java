...
public class Crucible {
	...
	public static void enterCrucibleEntrance(Player player) {
		if(!player.isTalkedWithMarv()) {
			player.getDialogueManager().startDialogue("SimpleMessage", "You need to check in with the Crucible's guardians at the other doorway first.");
			return;
		}
		if(player.getSkills().getCombatLevelWithSummoning() < 60) {
			player.getDialogueManager().startDialogue("SimpleMessage", "You need to be at least level 60 to enter Crucible.");
			return;
		}
	...
	public static boolean isImmune(Player player) {
		Long immune = (Long) player.getTemporaryAttributtes().get("CrucibleImmune");
	...
	public static void setImmune(Player player, int seconds) {
		if(seconds == 0)
			...
			player.getTemporaryAttributtes().put("CrucibleImmune", (Utils.currentTimeMillis() +  seconds*1000));
	}
	...
	public static Player getTarget(Player toPlayer) {
		...
		for(Player player : playersInside) {
			if (Math.abs(player.getSkills().getCombatLevelWithSummoning() - combatLevel) <= 10 && !isImmune(player, 9000))
				return player;
		}
		return null;
	}
	...
	private static final Fissures[] BANK_FISSURES = {Fissures.EAST_BANK, Fissures.NORTH_BANK, Fissures.WEST_BANK, Fissures.SOUTH_BANK};
	...
	public static void payBountyFee(Player player, CrucibleControler controler) {
		Fissures fissure = (Fissures) player.getTemporaryAttributtes().remove("crucibleBounty");
		if(fissure == null)
			return;
	...
	private static enum Fissures {
		EAST_BANK(3209, 6144, 4),
		NORTH_BANK(3263, 6198, 5),
		WEST_BANK(3318, 6144, 6),
		SOUTH_BANK(3260, 6089, 7),
		FISSURE_6(3266, 6132, 8),
		FISSURE_7(3294, 6118, 9),
		FISSURE_3(3279, 6151, 10),
		FISSURE_2(3287, 6173, 11),
		FISSURE_1(3259, 6183, 12),
		FISSURE_4(3248, 6155, 13),
