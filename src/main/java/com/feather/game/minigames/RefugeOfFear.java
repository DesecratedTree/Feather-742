...
public final class RefugeOfFear extends Controler {
	...
	public void start() {
		...
		if (!player.isExtremeDonator()) {
			...
			player.getDialogueManager().startDialogue("SimpleMessage", "You cannot take " + ItemDefinitions.getItemDefinitions(itemid).getName().toLowerCase() + " into Refuge of Fear.");
			player.getControlerManager().removeControlerWithoutCheck();
			return;
		}
	...
	public boolean processButtonClick(int interfaceId, int componentId, int slotId, int packetId) {
		...
		if (interfaceId == Inventory.INVENTORY_INTERFACE) {
			if (Food.forId(item.getId()) != null || Pots.getPot(item.getId()) != null) {
				player.getDialogueManager().startDialogue("SimpleMessage", "You cannot eat food or drink potions in this area!");
				return false;
			}
			...
			CoresManager.fastExecutor.schedule(new TimerTask() {
				...
				public void run() {
					if (!player.getInventory().containsItem(finalKey, 1)) {
						...
						player.getPackets().sendGameMessage("You get hurt by a" + (finalKey == 13073 ? "n " : " ") + finalDoorType);
						return;
					}
					player.getInventory().deleteItem(finalKey, 1);
					player.getPackets().sendGameMessage("You hear a click and the door unlocks.");
					player.getPackets().sendGameMessage("Something about this key protects you against the sudden " + finalDoorType);
					if(cutscene != null) {
	...
	public static enum Bosses {
		...
		FEAR_BOSS(15172, 80, 990, 0, "Master of Fear"),
		POWER_BOSS(15173, 80, 990, 0, "Master of Power"),
		DARKNESS_BOSS(15175, 80, 990, 0, "Master of Darkness"),
		DEATH_BOSS(15176, 80, 990, 0, "Master of Death");
