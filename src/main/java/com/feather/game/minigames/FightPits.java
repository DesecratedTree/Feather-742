...
public final class FightPits {
	...
	public static void leaveArena(Player player, int type) {
		synchronized (lock) {
			...
			if(type == 0) {
				...
				player.getPackets().closeInterface(player.getInterfaceManager().hasResizableScreen() ? 34 : 0);
				if(player.hasSkull() && player.getSkullId() == 1) {//if has champion skull
					player.removeSkull();
					player.getDialogueManager().startDialogue("SimpleNPCMessage", THHAAR_MEJ_KAH, "Well done in the pit, here take TokKul as reward.");
	...
	public static void setChampion() {
		...
		champion.getDialogueManager().startDialogue("SimpleNPCMessage", THHAAR_MEJ_KAH, "Well done, you were the last person in the pit and won that fight! The next round will start soon, wait for my signal before fighting.");
	}
