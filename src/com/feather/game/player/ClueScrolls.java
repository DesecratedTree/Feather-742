...
public class ClueScrolls {
	...
	private enum Riddles {
		...
		Riddle5(28, new int[] {2884,3449,2898,3438}, new String[] {"This reward will require a bit.","For the first thing you will","need to be by a den,","and you have to be a rouge.","You must have an idea outside","of its entrance to get the reward!","",""});//Mudsckipper Point
		int[] locations;
		String[] riddles;
		int emoteid;
	...
	public static void showObjectMap(Player p, ObjectMaps objmap){
		p.getPackets().sendInterface(false, p.getInterfaceManager().hasResizableScreen() ? 746:548, p.getInterfaceManager().hasResizableScreen() ? 28:27, objmap.interid);
		p.getPackets().sendGameMessage(objmap.hint);
		
	}
	
	public static void showRiddle(Player p, Riddles riddle){
		p.getPackets().sendInterface(false, p.getInterfaceManager().hasResizableScreen() ? 746:548, p.getInterfaceManager().hasResizableScreen() ? 28:27, 345);
	...
	public static void showMap(Player p, Maps map){
		p.getPackets().sendInterface(false, p.getInterfaceManager().hasResizableScreen() ? 746:548, p.getInterfaceManager().hasResizableScreen() ? 28:27, map.interfaceId);
	...
	public static void giveReward(Player p){
		...
		if (random > 875){
			...
			p.getPackets().sendGameMessage("You've found another clue inside the chest.");
			p.getInventory().deleteItem(2717,1);
