...
public final class DominionTower implements Serializable {
	...
	public static final class Boss {
		...
		public Boss(String name, String text, int voice, boolean forceMulti, Item item, int[] arena, 
				int... ids) {
			this.name = name;
			this.text = text;
		...
		CoresManager.slowExecutor.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					RegionBuilder.copyAllPlanesMap(BOSSES[getNextBossIndex()].arena[0], BOSSES[getNextBossIndex()].arena[1]
