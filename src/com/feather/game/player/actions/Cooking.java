...
public class Cooking extends Action {
	...
	public enum Cookables {
		...
		static {
			for (Cookables ingredient : Cookables.values()) {
				ingredients.put((short) ingredient.getRawItem().getId(),
						ingredient);
			}
		}
		...
		private Cookables(Item raw, int lvl, int burningLvl, int exp,
	...
	public boolean process(Player player) {
		...
		if (player.getSkills().getLevel(Skills.COOKING) < cook.getLvl()) {
			player.getDialogueManager().startDialogue("SimpleMessage",
					"You need a level of " + cook.getLvl() + " to cook this.");
			return false;
		}
		return true;
	}
	...
	public int processWithDelay(Player player) {
		...
		if ((player.getSkills().getLevel(Skills.COOKING) >= cook
				.getBurningLvl()) ? false : isBurned(cook, player)) {
