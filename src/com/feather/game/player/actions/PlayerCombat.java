...
import com.feather.game.npc.familiar.Steeltitan;
import com.feather.game.npc.fightkiln.HarAken;
import com.feather.game.npc.fightkiln.HarAkenTentacle;
import com.feather.game.npc.godwars.zaros.NexMinion;
import com.feather.game.npc.qbd.QueenBlackDragon;
import com.feather.game.player.CombatDefinitions;
...
public class PlayerCombat extends Action {
	...
	public int mageAttack(final Player player, int spellId, boolean autocast) {
		...
		if (spellId == 65535) {
			...
			mage_hit_gfx = 2036;
			delayMagicHit(2,
					getMagicHit(player, getRandomMagicMaxHit(player, (5 * player.getSkills().getLevel(Skills.MAGIC)) - 180)));
	 ...
	 */
	/*public int getMagicMaxHit(Player player, int baseDamage) { //older formula
		double effectiveAttack = (Math.round((player.getSkills().getLevel(Skills.MAGIC) * player.getPrayer().getMageMultiplier())) + 8)
				+ player.getCombatDefinitions().getBonuses()[CombatDefinitions.MAGIC_ATTACK];
		if (fullVoidEquipped(player, new int[] { 11663, 11674 }))
			effectiveAttack *= 1.3;
		effectiveAttack *= player.getAuraManager().getMagicAccurayMultiplier();
		double totalDefence = 0;
		if (target instanceof Player) {
			Player p2 = (Player) target;
			double effectiveDefence = (Math.round(p2.getSkills().getLevel(Skills.DEFENCE) + (p2.getPrayer().getDefenceMultiplier() + Utils.random(0, 14))) + 8) + p2.getCombatDefinitions().getBonuses()[CombatDefinitions.MAGIC_DEF];
			double effectiveMagicDefence = p2.getSkills().getLevel(Skills.MAGIC) + p2.getPrayer().getMageMultiplier();
			effectiveMagicDefence *= 0.7;
			effectiveDefence *= 0.3;
			totalDefence = Math.round(effectiveMagicDefence + effectiveDefence);
		} else {
			NPC n = (NPC) target;
			totalDefence = n.getBonuses() != null ? n.getBonuses()[CombatDefinitions.MAGIC_DEF] : 0;
		}
		double attack = Math.round((effectiveAttack * (64 + player.getCombatDefinitions().getBonuses()[CombatDefinitions.MAGIC_ATTACK])) / 10);
		double defence = Math.round(((totalDefence * (64 + player.getCombatDefinitions().getBonuses()[CombatDefinitions.MAGIC_DEF])) / 10));
		double accuracy = 0.5;
		if (attack < defence)
	...
	/*public int getMagicMaxHit(Player player, int baseDamage) {
		double EA = (Math.round(player.getSkills().getLevel(Skills.MAGIC) * player.getPrayer().getMageMultiplier()) + 8);
		if (fullVoidEquipped(player, new int[] { 11663, 11674 }))
			EA *= 1.3;
		...
		if (target instanceof Player) {
			Player p2 = (Player) target;
			double EMD = p2.getSkills().getLevel(Skills.MAGIC) * p2.getPrayer().getMageMultiplier();
			if (p2.getFamiliar() != null && (p2.getFamiliar().getId() == 6870 || p2.getFamiliar().getId() == 6871))
				EMD *= 1.05;
			Math.round(EMD);
			double ERD = (Math.round(p2.getSkills().getLevel(Skills.DEFENCE) * p2.getPrayer().getDefenceMultiplier()) + 8) + (player.getCombatDefinitions().isDefensiveCasting() ? 3 : 0);
			EMD *= 0.7;
			ERD *= 0.3;
			ED = (EMD + ERD);
			...
			NPC n = (NPC) target;
			DB = ED = n.getBonuses() != null ? n.getBonuses()[CombatDefinitions.MAGIC_DEF] : 0;
		}
		double A = (EA * (64 + player.getCombatDefinitions().getBonuses()[CombatDefinitions.MAGIC_ATTACK])) / 10;
		double D = (ED * (64 + DB)) / 10;
	...
	/*	private double getMagicDefence(Entity e) {
		...
		int style = p != null ? 0 : 1;
		if (p != null) {
			int type = CombatDefinitions.getXpStyle(p.getEquipment().getWeaponId(), p.getCombatDefinitions().getAttackStyle());
			style += type == Skills.DEFENCE ? 3 : type == CombatDefinitions.SHARED ? 1 : 0;
		}
		double defLvl = (p != null ? p.getSkills().getLevel(Skills.DEFENCE) : ((NPC) e).getCombatLevel() * 0.7) * 0.3;
		defLvl += (p != null ? p.getSkills().getLevel(Skills.MAGIC) : ((NPC) e).getCombatLevel() * 0.7) * 0.7;
		int defBonus = p != null ? p.getCombatDefinitions().getBonuses()[CombatDefinitions.MAGIC_DEF] 
				: ((NPC) e).getBonuses() != null ? ((NPC) e).getBonuses()[CombatDefinitions.MAGIC_DEF] : 5;
		double defMult = 1.0;
		if (p != null) {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			if (weaponId != -1) {
				...
				if (weaponName.contains("throwing axe")
					...
					delayHit(2, weaponId, attackStyle,
							getRangeHit(player, hit));
					checkSwiftGlovesEffect(player, 2, attackStyle, weaponId, hit, getArrowProjectileGfxId(weaponId, ammoId), 
							41, 36, 20, 35, 16, 0);
	...
	public int getRandomMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean defenceAffects, double specMultiplier, boolean usingSpec) {
		...
		if (defenceAffects) {
			accuracyMultiplier = getSpecialAccuracyModifier(player);
		}
		if (ranging && defenceAffects) {
			double accuracy = accuracyMultiplier * getRangeAccuracy(player, attackStyle) + 1;
			double defence = getRangeDefence(target) + 1;
			if (r.nextInt((int) accuracy) < r.nextInt((int) defence)) {
				return 0;
			}
		} else if (defenceAffects) {
			double accuracy = accuracyMultiplier * getMeleeAccuracy(player, attackStyle, weaponId) + 1;
			double defence = getMeleeDefence(target, attackStyle, weaponId) + 1;
			if (r.nextInt((int) accuracy) < r.nextInt((int) defence)) {
				return 0;
			}
		}
		int hit = r.nextInt(max_hit + 1);
		if (target instanceof NPC) {
			NPC n = (NPC) target;
			if (n.getId() == 9463 && hasFireCape(player))
				hit += 40;
	 ...
	 */
	/*	public static double getMeleeAccuracy(Player e, int attackStyle, int weaponId) {
		int style = attackStyle == 0 ? 3 : attackStyle == 2 ? 1 : 0;
		int attLvl = e.getSkills().getLevel(Skills.ATTACK);
		int attBonus = e.getCombatDefinitions().getBonuses()[CombatDefinitions.getMeleeBonusStyle(weaponId, attackStyle)];
		if (weaponId == -2) {
			attBonus += 82;
		}
		double attMult = 1.0 * e.getPrayer().getAttackMultiplier();
		double accuracyMultiplier = 1.0;
		if (fullVoidEquipped(e, 11665, 11676))  {
			accuracyMultiplier *= 0.15;
		}
		double cumulativeAtt = attLvl * attMult + style;
		return (14 + cumulativeAtt + (attBonus / 8) + ((cumulativeAtt * attBonus) / 64)) * accuracyMultiplier;
	}
	...
	/*	public static double getMeleeMaximum(Player e, int attackStyle, int weaponId) {
		...
		int strBonus = e.getCombatDefinitions().getBonuses()[CombatDefinitions.STRENGTH_BONUS];
		if (weaponId == -2) {
			strBonus += 82;
		}
		double strMult = e.getPrayer().getStrengthMultiplier();
		double xpStyle = CombatDefinitions.getXpStyle(weaponId, attackStyle);
		double style = xpStyle == Skills.STRENGTH ? 3 : xpStyle == CombatDefinitions.SHARED ? 1 : 0;
		int dhp = 0;
		double dharokMod = 1.0;
		double hitMultiplier = 1.0;
		if (fullDharokEquipped(e)) {
			dhp = e.getMaxHitpoints() - e.getHitpoints();
			dharokMod = (dhp * 0.001) + 1;
		}
		if (fullVoidEquipped(e, 11665, 11676)) {
			hitMultiplier *= 1.1;
	...
	/*	public static double getMeleeDefence(Entity e, int attackStyle, int weaponId) {
		...
		int style = player ? ((Player) e).getCombatDefinitions().getAttackStyle() : 0;
		style = style == 2 ? 1 : style == 3 ? 3 : 0;
		int defLvl = player ? ((Player) e).getSkills().getLevel(Skills.DEFENCE) : (int) (((NPC) e).getCombatLevel() * 0.6);
		int defBonus = player ? ((Player) e).getCombatDefinitions().getBonuses()[
		      CombatDefinitions.getMeleeDefenceBonus(CombatDefinitions.getMeleeBonusStyle(weaponId, attackStyle))] : 0;
		if (!player) {
			defBonus = ((NPC) e).getBonuses() != null ? ((NPC) e).getBonuses()[
			                 CombatDefinitions.getMeleeDefenceBonus(CombatDefinitions.getMeleeBonusStyle(weaponId, attackStyle))] : 0;
		}
		double defMult = 1.0 * (player ? ((Player) e).getPrayer().getDefenceMultiplier() : 1.0);
	 ...
	 */
	/*	public static double getRangeAccuracy(Player e, int attackStyle) {
		int style = attackStyle == 0 ? 3 : attackStyle == 2 ? 1 : 0;
		int attLvl = e.getSkills().getLevel(Skills.RANGE);
		int attBonus = e.getCombatDefinitions().getBonuses()[4];
		double attMult = 1.0 * e.getPrayer().getRangeMultiplier() * e.getAuraManager().getRangeAccurayMultiplier();
		double accuracyMultiplier = 1.05;
		if (fullVoidEquipped(e, 11664, 11675)) {
	 ...
	 */
	/*	public static double getRangeMaximum(Player e, int attackStyle) {
		int style = attackStyle == 0 ? 3 : attackStyle == 2 ? 1 : 0;
		int strLvl = e.getSkills().getLevel(Skills.RANGE);
		int strBonus = e.getCombatDefinitions().getBonuses()[CombatDefinitions.RANGED_STR_BONUS];
		double strMult = 1.0 * e.getPrayer().getRangeMultiplier();
		double hitMultiplier = 1.0;
		if (fullVoidEquipped(e, 11664, 11675)) {
			hitMultiplier += 0.1;
		}
		double cumulativeStr = strLvl * strMult + style;
		return (14 + cumulativeStr + (strBonus / 8) + ((cumulativeStr * strBonus) / 64)) * hitMultiplier;
	}
	 ...
	 */
	/*	public static double getRangeDefence(Entity e) {
		boolean player = e instanceof Player;
		int style = player ? ((Player) e).getCombatDefinitions().getAttackStyle() : 0;
		style = style == 2 ? 1 : style == 3 ? 3 : 0;
		int defLvl = player ? ((Player) e).getSkills().getLevel(Skills.DEFENCE) : (int) (((NPC) e).getCombatLevel() * 0.6);
		int defBonus = player ? ((Player) e).getCombatDefinitions().getBonuses()[CombatDefinitions.RANGE_DEF] : 0;
		if (!player) {
			defBonus = ((NPC) e).getBonuses() != null ? ((NPC) e).getBonuses()[9] : 0;
		}
		double defMult = 1.0 * (player ? ((Player) e).getPrayer().getDefenceMultiplier() : 1.0);
		double cumulativeDef = defLvl * defMult + style;
		return 14 + cumulativeDef + (defBonus / 8) + ((cumulativeDef * defBonus) / 64);
	...
	/*	public static final int getMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean usingSpec, double specMultiplier) {
		if (ranging) {
			return (int) (getRangeMaximum(player, attackStyle) * specMultiplier);
		}
		return (int) (getMeleeMaximum(player, weaponId, attackStyle) * specMultiplier);
	}*/
