...
public class NomadsRequiem extends Controler {
	...
	public static enum DungeonPart {
		ENTRANCE(409, 733, 7, 3, 46, 12, 23, 21),
		HALL(408, 736, 8, 8, 31, 2, 31, 15),
		THRONE(418, 730, 5, 5, 16, 2);
...
/*	private void teletoDoor(Player player, int id) {
		player.setNextWorldTile(new WorldTile(baseDoors[id*2], baseDoors[id*2]+1, 0));
	}*/
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if(stage == 0) {
					...
					Dialogue.sendNPCDialogueNoContinue(player, nomad.getId(), 9827, "You don't stand a chance "+player.getDisplayName()+"!");
				}else if (stage == 6) {
					...
					nomad.finish();
					player.getDialogueManager().startDialogue("SimplePlayerMessage", "Maybe I should follow that guy..");
					Dialogue.closeNoContinueDialogue(player);
					player.unlock();
					stop();
				}
		...
		CoresManager.slowExecutor.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					mapBaseChunks = RegionBuilder.findEmptyChunkBound(part.sizeX, part.sizeY);
					currentPart = part;
					RegionBuilder.copyAllPlanesMap(part.chunkX, part.chunkY, mapBaseChunks[0], mapBaseChunks[1], part.sizeX, part.sizeY);
