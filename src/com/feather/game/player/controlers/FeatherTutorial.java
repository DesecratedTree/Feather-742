...
public final class FeatherTutorial extends Controler {
	...
	public void refreshStage() {
		...
		if (stage == 0 || stage == 1 || stage == 2) {
			...
			player.getHintIconsManager().addHintIcon(3089, 3092, 0, 125, 4, 0,
					-1, false);
		}else if (stage == 19) {
			player.getHintIconsManager().addHintIcon(3078, 3084, 0, 125, 4, 0,
					-1, false);
		}else if (stage == 20) {
	...
	public boolean canAddInventoryItem(int itemId, int amount) {
		...
		}else if (getStage() == 14 && (itemId == 7954 || itemId == 315)) {
			updateProgress();
	...
	public boolean processObjectClick1(GameObject object) {
		if (object.getId() == 3014 && object.getX() == 3098
			...
			player.stopAll();
			player.addWalkSteps(
					player.getX() > object.getX() ? object.getX()
							: object.getX() + 1, player.getY(), -1, false);
			return false;
			
		}else if (object.getId() == 3017 && object.getX() == 3079
				&& object.getY() == 3084) {
			int stage = getStage();
	...
	public void sendInterfaces() {
		int stage = getStage();
		boolean rezi = player.getInterfaceManager().hasResizableScreen();
