...
import com.feather.game.npc.fightkiln.FightKilnNPC;
import com.feather.game.npc.fightkiln.HarAken;
import com.feather.game.npc.fightkiln.TokHaarKetDill;
...
public class FightKiln extends Controler {
	...
	public void buildMap() {
		int[] map = getMap();
		RegionBuilder.copyAllPlanesMap(map[0], map[1], boundChuncks[0], boundChuncks[1], 8);
	}
...
42 42 maxX, maxY
	...
	public void startWave() {
		if(stage != Stages.RUNNING)
			return;
		int currentWave = getCurrentWave();
		player.getInterfaceManager().sendTab(player.getInterfaceManager().hasResizableScreen() ? 11 : 0, 316);
	...
	public void spawn(int index) {
		...
		for(int i = index; i <  (index + 4 > WAVES[currentWave-1].length ? WAVES[currentWave-1].length  : index + 4); i++) {
			...
			if(npcId == 15213)
				new	TokHaarKetDill(WAVES[currentWave-1][i], getSpawnTile(i, NPCDefinitions.getNPCDefinitions(npcId).size), this);
			else
				new FightKilnNPC(WAVES[currentWave-1][i], getSpawnTile(i, NPCDefinitions.getNPCDefinitions(npcId).size), this);
	...
	private int[] getLavaCrystal() {
		...
		case 35: return new int[] {23657, 23658};
		default: return null;
		}
	}
	...
	public static void useCrystal(final Player player, int id) {
		...
		case 23653: //invulnerability
			...
			CoresManager.slowExecutor.schedule(new Runnable() {
				...
				public void run() {
					player.getTemporaryAttributtes().remove("FightKilnCrystal");
					player.getPackets().sendGameMessage("<col=7E2217>The power of the crystal dwindles and you're vulnerable once more.");
					player.setInvulnerable(false);
				}
			}, 15, TimeUnit.SECONDS);
			break;
		...
		case 23658: //CONSTITUTION
			...
			CoresManager.slowExecutor.schedule(new Runnable() {
				...
				public void run() {
					try {
					player.getTemporaryAttributtes().remove("FightKilnCrystal");
					player.getPackets().sendGameMessage("<col=7E2217>The power of the crystal dwindles and your constitution prowess returns to normal.");
					player.setHpBoostMultiplier(0);
					player.getEquipment().refreshConfigs(false);
					} catch (Throwable e) {
						Logger.handle(e);
	...
	private static void boostCrystal(final Player player, final int skill) {
		...
		if(skill == Skills.RANGE)
			player.getPackets().sendGameMessage("<col=7E2217>The power of the crystal improves your Ranged prowess, at the expense of your Defence, Strength and Magical ability.");
