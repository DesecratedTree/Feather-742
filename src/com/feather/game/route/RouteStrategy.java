...
public abstract class RouteStrategy {
	 ...
	 */
	public abstract boolean canExit(int currentX, int currentY, int sizeXY,
	 ...
	 */
	protected static boolean checkWallDecorationInteract(int[][] clip,
			int currentX, int currentY, int sizeXY, int targetX, int targetY,
			int targetType, int targetRotation) {
	 ...
	 */
	protected static boolean checkWallInteract(int[][] clips, int currentX,
			int currentY, int sizeXY, int targetX, int targetY, int targetType,
			int targetRotation) {
		// TODO refactor
		if (sizeXY == 1) {
			if (currentX == targetX && currentY == targetY)
				return true; // we are inside the object
		} else if (targetX >= currentX && targetX <= currentX + sizeXY - 1
				&& targetY <= targetY + sizeXY - 1)
		...
		if (sizeXY == 1) {
			if (targetType == 0) {
				if (targetRotation == 0) {
					...
					if (currentX == targetX && targetY + 1 == currentY
							&& (clips[currentX][currentY] & 0x2c0120) == 0)
						return true;
					if (targetX == currentX && currentY == targetY - 1
							&& (clips[currentX][currentY] & 0x2c0102) == 0)
						return true;
				} else if (targetRotation == 1) {
					if (currentX == targetX && targetY + 1 == currentY)
						return true;
					if (currentX == targetX - 1 && targetY == currentY
							&& (clips[currentX][currentY] & 0x2c0108) == 0)
						return true;
					if (targetX + 1 == currentX && currentY == targetY
							&& (clips[currentX][currentY] & 0x2c0180) == 0)
						return true;
				} else if (targetRotation == 2) {
					if (targetX + 1 == currentX && currentY == targetY)
						return true;
					...
					if (targetX - 1 == currentX && currentY == targetY
							&& (clips[currentX][currentY] & 0x2c0108) == 0)
						return true;
					if (targetX + 1 == currentX && targetY == currentY
							&& (clips[currentX][currentY] & 0x2c0180) == 0)
						return true;
				}
			}
			...
			if (targetType == 9) {
				if (targetX == currentX && targetY + 1 == currentY
						&& (clips[currentX][currentY] & 0x20) == 0)
					return true;
				if (currentX == targetX && targetY - 1 == currentY
						&& (clips[currentX][currentY] & 0x2) == 0)
					return true;
				if (currentX == targetX - 1 && currentY == targetY
						&& (clips[currentX][currentY] & 0x8) == 0)
					return true;
				if (currentX == targetX + 1 && currentY == targetY
						&& (clips[currentX][currentY] & 0x80) == 0)
					return true;
			}
		} else {
			...
			if (targetType == 0) {
				if (targetRotation == 0) {
					if (targetX - sizeXY == currentX && targetY >= currentY
							&& targetY <= i_67_)
						return true;
					if (targetX >= currentX && targetX <= i_66_
							&& currentY == targetY + 1
							&& (clips[targetX][currentY] & 0x2c0120) == 0)
						return true;
					if (targetX >= currentX && targetX <= i_66_
							&& targetY - sizeXY == currentY
							&& (clips[targetX][i_67_] & 0x2c0102) == 0)
						return true;
				} else if (targetRotation == 1) {
					if (targetX >= currentX && targetX <= i_66_
							&& targetY + 1 == currentY)
						return true;
					if (currentX == targetX - sizeXY && targetY >= currentY
							&& targetY <= i_67_
	 ...
	 */
	protected static boolean checkFilledRectangularInteract(int[][] clip,
			int currentX, int currentY, int sizeX, int sizeY, int targetX,
			int targetY, int targetSizeX, int targetSizeY, int accessBlockFlag) {
		// TODO refactor
