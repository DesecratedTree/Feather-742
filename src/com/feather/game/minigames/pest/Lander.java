package com.feather.game.minigames.pest;
...
public class Lander {
	...
	public void enterLander(Player player) {
		if (player.getSkills().getCombatLevel() < landerRequirement.getRequirement()) {
			player.getDialogueManager().startDialogue("SimpleMessage", "You need a combat level of "+ landerRequirement.getRequirement() +" or more to enter in boat.");
			return;
		}
		synchronized(lock) {
			if (lobby.size() == 0) // :O ?
