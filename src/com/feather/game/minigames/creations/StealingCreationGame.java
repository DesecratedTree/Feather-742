...
public class StealingCreationGame extends Controler {
	...
	public boolean processObjectClick1(GameObject object) {
		...
		case 39550:
			...
			StealingCreation.startDynamicSkill(player, object, name == null ? null : StealingCreation.getAnimationForBase(Integer.parseInt(name.substring(name.indexOf("(")).replace("(class ", "").replace(")", "")) - 1, 1),  baseId, calculatedId);
			return false;
		case 39586:
		case 39587:
		case 39588:
		case 39589:
			...
			if (item.getId() != -1)
				name = item.getDefinitions().getName();
			StealingCreation.startDynamicSkill(player, object, name == null ? null : StealingCreation.getAnimationForBase(Integer.parseInt(name.substring(name.indexOf("(")).replace("(class ", "").replace(")", "")) - 1, 0),  baseId, calculatedId);
			return false;
		case 39574:
		case 39575:
		case 39576:
		case 39577:
			...
			if (item.getId() != -1)
				name = item.getDefinitions().getName();
			StealingCreation.startDynamicSkill(player, object, name == null ? null : StealingCreation.getAnimationForBase(Integer.parseInt(name.substring(name.indexOf("(")).replace("(class ", "").replace(")", "")) - 1, 3),  baseId, calculatedId);
			return false;
		case 39569:
		case 39564:
		case 39563:
		case 39562:
			...
			if (item.getId() != -1)
				name = item.getDefinitions().getName();
			StealingCreation.startDynamicSkill(player, object, name == null ? null : StealingCreation.getAnimationForBase(Integer.parseInt(name.substring(name.indexOf("(")).replace("(class ", "").replace(")", "")) - 1, 2),  baseId, calculatedId);
			return false;
		}
		return true;
	}
	...
	private void processKilnExchange(int componentId, int packetId) {
		...
		if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET)
			...
			amount = player.getInventory().getNumberOf(itemId);
