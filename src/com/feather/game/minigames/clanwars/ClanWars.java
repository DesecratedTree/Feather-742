...
public final class ClanWars implements Serializable {
	...
	public void switchRule(Rules rule, Player player) {
		...
		if (other == null 
				|| player.getTemporaryAttributtes().get("clan_wars") != other.getTemporaryAttributtes().get("clan_wars")) {
			return;
		}
		if (rule == Rules.NO_MAGIC) {
			if (get(Rules.NO_RANGE) && get(Rules.NO_MELEE)) {
				player.getPackets().sendGameMessage("You can't activate all combat style rules, how would you fight?");
				return;
			} else {
			...
			CoresManager.slowExecutor.submit(new Runnable() {
				...
				public void run() {
					...
					for (Player p : firstTeam.getPlayers()) {
						if (p != player && p != other) {
							p.getPackets().sendGameMessage("<col=FF0000>Your clan has been challenged to a clan war!</col>");
							p.getPackets().sendGameMessage("<col=FF0000>Step through the purple portal in the Challenge Hall.</col>");
	...
	public static void enter(Player p) {
		...
		final ClanWars c = hasWar ? p.getCurrentFriendChat().getClanWars() : (ClanWars) p.getTemporaryAttributtes().get("view_clan");
		if (c == null) {
			return;
		}
		...
		p.getInterfaceManager().sendTab(p.getInterfaceManager().hasResizableScreen() ? 11 : 27, 265);
		...
		Integer prefix = (Integer) p.getTemporaryAttributtes().get("view_prefix");
		if (prefix == null || prefix == 0) {
			c.timer.refresh(p, true);
			c.firstViewers.add(p);
			p.setNextWorldTile(c.baseLocation.transform(c.areaType.getFirstDeathOffsetX(), c.areaType.getFirstDeathOffsetY(), 0));
		} else {
			c.timer.refresh(p, false);
	...
	public void updateWar() {
		...
		} else if (timer.isStarted() && !isMostKills() && ((kills & 0xFFFF) >= victoryType || (kills >> 24 & 0xFFFF) >= victoryType)) {
			timer.cancel();
			endWar();
		} else {
	...
	public void endWar() {
		...
		for (Player player : firstPlayers) {
			...
			boolean resized = player.getInterfaceManager().hasResizableScreen();
		...
		CoresManager.slowExecutor.schedule(new Runnable() {
			...
			public void run() {
				int width = (areaType.getNorthEastTile().getX() - areaType.getSouthWestTile().getX()) / 8 + 1;
				int height = (areaType.getNorthEastTile().getY() - areaType.getSouthWestTile().getY()) / 8 + 1;
				RegionBuilder.destroyMap(baseLocation.getChunkX(), baseLocation.getChunkY(), width, height);
			}			
