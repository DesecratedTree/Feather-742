...
import com.feather.game.DynamicRegion;
...
import com.feather.game.player.HintIcon;
...
public class WorldPacketsEncoder extends Encoder {
	...
	public void sendJoinClanChat(String username, String clanName) {
		...
		stream.writeByte(1);
		if (!player.inClanChat) {
			int idk = 2;
			int idk3 = 1;
			boolean idk1 = false;
			boolean idk2 = false;
			stream.writeByte(idk);
			if (idk == 1) {
				idk1 = true;
			}
			if (idk == 2) {
			...
			stream.writeLong(0L);
			stream.writeString(clanName);
			stream.writeByte(0);
			stream.writeByte(0);
			stream.writeByte(-1);
			stream.writeShort(idk3);
			if (idk3 > 0) {
				for(int i = 0; i < idk3; i++) {
					if (idk1) {
						stream.writeLong(0L);//idk probably long username
					}
					if (idk2) {
			...
			player.getPackets().sendGameMessage("You join the " + clanName + " clan chat.");
			player.inClanChat = true;
		} else {
			player.getPackets().sendGameMessage("You leave the " + clanName + " clan chat.");
	...
	public void sendInventoryMessage(int border, int object, String message) {
		sendGameMessage(message);
		sendRunScript(948, border, object, message);
	}
