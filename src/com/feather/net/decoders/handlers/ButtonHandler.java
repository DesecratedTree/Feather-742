...
import com.feather.game.player.content.XPLamp;
...
public class ButtonHandler {
	...
	public static void handleButtons(final Player player, InputStream stream,
		...
		if (interfaceId == 548 || interfaceId == 746) {
			if ((interfaceId == 548 && componentId == 148)
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON3_PACKET) {
					player.getPouch().sendExamine();
				}
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON2_PACKET) {
					player.getPackets().sendRunScript(108,
							new Object[] { "Your money pouch contains " + player.getPouch().getMoney() + " coins. How many would you like to withdraw?" });
					player.getTemporaryAttributtes().put("moneypouch",
							Boolean.TRUE);
				}
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET && player.hasOpen == false) {
					player.getPouch().toggleMoneyPouch();
					player.getPouch().refreshPouch(true);
					player.getPackets().sendSound(1295, 0, 1);
					player.hasOpen = true;
				} else if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET && player.hasOpen == true) {
					player.getPouch().toggleMoneyPouch();
					player.getPouch().refreshPouch(true);
					player.getPackets().sendSound(1295, 0, 1);
					player.hasOpen = false;
				}
			}
		} else if (interfaceId == 1110 || interfaceId == 1096) {
			ClanManager.handleButtons(player, interfaceId, componentId);
			...
			XPLamp.handleButtons(player, componentId);
		...
		} else if (interfaceId == 190 && componentId == 15 && slotId == 1) {
            ...
            if (componentId == 49) {
                ...
                player.getPackets().sendConfig(1078, 0);
            } else if (componentId == 201) {
                if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
                    if (player.isBuying) {
                        int slot = (Integer) player.getTemporaryAttributtes().get("BuySelectedSlot");
                        shop.buy(player, slot, shop.quantity);
                    } else {
                        int slot2 = (Integer) player.getTemporaryAttributtes().get("SellSelectedSlot");
                        shop.sell(player, slot2, shop.quantity);
                    }
                }
            } else if (componentId == 214) {
                if (shop.quantity < 2) {
                    player.getPackets().sendConfig(2564, shop.quantity);
                    return;
                }
                shop.quantity -= 1;
                player.getPackets().sendConfig(2564, shop.quantity);
...
//				player.getBank().depositAllMoneyPouch(true);
			...
			if (componentId == 44)
				player.getPackets().sendWindowsPane(
						player.getInterfaceManager().hasResizableScreen() ? 746
								: 548, 2);
